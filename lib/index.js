...
Mehdown.parse = function(text, opts) {
    ...
    text = text.replace(/<a href="https?:\/\/(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&(?:amp;)?v=))((?:\w|-){11})(?:\S*?)?(?:t=([0-9ms]+))?">https?:\/\/(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&(?:amp;)?v=))(?:(?:\w|-){11})(?:\S*?)?(?:t=(?:[0-9ms]+))?<\/a>/g, Mehdown.youtube);
...
Mehdown.youtube = function(match, p1, p2, offset, string) {
    if (p2 && p2 !== '' && p2 !== '$2') {
        var times = p2.split(/m/);
        var timeString;
        if (times[1] && times[1] !== '') {
            timeString = parseInt(times[0]) * 60 + parseInt(times[1]);
        }
            ...
            timeString = times[0] * 60;
        }
        console.log(timeString);
        return '<iframe allowfullscreen class="youtube" frameborder="0" src="//www.youtube.com/embed/' + p1 + '?start=' + timeString + '&autohide=1&color=white&showinfo=0&theme=light"></iframe>';
    }
    else {
        return '<iframe allowfullscreen class="youtube" frameborder="0" src="//www.youtube.com/embed/' + p1 + '?autohide=1&color=white&showinfo=0&theme=light"></iframe>';
    }
};
