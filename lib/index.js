...
Mehdown.parse = function(text, opts) {
    ...
    text = text.replace(/<a href="https?:\/\/(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&(?:amp;)?v=))((\w|-){11})(?:\S+)?">https?:\/\/(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&(?:amp;)?v=))((\w|-){11})(?:\S+)?<\/a>/g, Mehdown.youtube('$1'));
    ...
    // Archive.org games
    if (Mehdown.archiveGame) {
        text = text.replace(/<a href="https?:\/\/(?:www\.)?(?:archive\.org\/details\/)?(\w+)">https?:\/\/(?:www\.)?(?:archive\.org\/details\/)?(\w+)<\/a>/g, Mehdown.archiveGame);
    }
...
if (typeof exports === 'object' && typeof require === 'function') {
    ...
    Mehdown.archiveGame = function(match, game) {
        var res = request('GET', 'https://archive.org/details/' + game + '&output=json');

        body = JSON.parse(res.body.toString());

        if (body.metadata && body.metadata.emulator) {
            return '<iframe src="https://archive.org/embed/' + game + '" width="560" height="384" frameborder="0" webkitallowfullscreen="true" mozallowfullscreen="true" allowfullscreen></iframe>';
        } else if (body.metadata && body.metadata.description) {
            return '<a href="https://archive.org/details/' + game + '">' + (body.metadata.description) + '</a>';
        } else {
            return match;
        }
    };
