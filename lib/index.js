...
Mehdown.parse = function(text, opts) {
    ...
    text = text.replace(/<a href="https?:\/\/(?:www\.)?(?:twitter\.com\/)(?:#!\/)?(\w+)\/status(es)?\/(\d+)">https?:\/\/(?:www\.)?(?:twitter\.com\/)(?:#!\/)?(\w+)\/status(es)?\/(\d+)<\/a>/gi, Mehdown.twitter('$1', 'p3'));
    ...
    //grab the start time of the youtube video if it exists
    var timeString = text.split(/#t=/)[1];
    var time;
    if (timeString) {
    ...
    text = text.replace(/<a href="https?:\/\/(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&(?:amp;)?v=))((\w|-){11})(?:\S+)?(t=([0-9ms]+))">https?:\/\/(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&(?:amp;)?v=))((\w|-){11})(?:\S+)?(t=([0-9ms]+))<\/a>/g, Mehdown.youtube('$1', '$3'));
...
function youtubeParse(match, p1, offset, string) {
    return Mehdown.youtube(p1, '');
}

function youtubeParse(match, p1, p2, p3, offset, string) {
    console.log('p3: ' + p3);
    p3 = p3.substring(2);
    var times = p3.split(/[ms]/);
    var time;
