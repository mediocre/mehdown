    ...
    define: function(args, callback) {
        if (!args || args === '--help' || args === '-h') {
            ...
            var dict = new Dictionary({
                app_id: process.env.OXFORD_DICTIONARY_APP_ID,
                app_key: process.env.OXFORD_DICTIONARY_APP_KEY,
                source_lang: 'en'
            });
            ...
            async.retry(function(callback) {
                        ...
                        le.entries.forEach(e => {
                            ...
                            e.senses.forEach((s) => {
                                s.definitions.forEach(d => {
                                    ...
                                    if (definitionCount % 2 === 1) {
                                        _markdown += `- **${definitionCount}** ${d}\n`;
                                    } else {
                                        _markdown += `+ **${definitionCount}** ${d}\n`;
                                    }
                                    ...
                                    if (s.subsenses) {
                                        s.subsenses.forEach((ss, j) => {
                                            ss.definitions.forEach(d => {
                                                if (j < 3) {
                                                    _markdown += `  + **${definitionCount}.${j + 1}** ${d}\n`;
                                                } else {
                                                    hasMoreDefinitions = true;
                                                }
                                            });
                ...
                if (hasMoreDefinitions) {
                    _markdown += `\n\n More definitions available at [Oxford Dictionaries](https://en.oxforddictionaries.com/definition/${encodeURIComponent(args)})\n`;
                }
...
exports.html = {
    ...
    removeImageSizeAttributes: function(html) {
        ...
        imgElements.forEach(function(imgElement) {
            var imgElementWithoutSizeAttributes = exports.html.removeAttribute(imgElement, 'height');
