...
const commands = {
    ...
    emojify: function(args, callback) {
        ...
        var tokens = new natural.WordPunctTokenizer().tokenize(args);

        tokens.forEach(t => {
            var stem = natural.PorterStemmer.stem(t.toLowerCase());

            for (var key in emoji) {
                var e = natural.PorterStemmer.stem(key.toLowerCase());

                if (e === stem) {
                    args = args.replace(new RegExp(t, 'g'), emoji[key].shortname);
                    return;
                }
            }

            for (key in emoji) {
                e = natural.PorterStemmer.stem(key.split('_')[0].toLowerCase());
