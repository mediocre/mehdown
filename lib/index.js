...
const commandRegExp = /^(?:@\w{1,15}\s+)*?\s*\/(\w+)\s?(.*)$/gim;
...
exports.render = function(markdown, options, callback) {
    ...
    async.series([
        function _commands(callback) {
            ...
            var splitMarkdown = markdown.split(/(?=^(?:@\w{1,15}\s+)*?\s*\/(?:\w+)\s?(?:.*)$)/gmi);
            console.log(splitMarkdown);
            ...
            async.forEachOf(commandMatches, function(commandMatch, index, callback) {
                ...
                // Execute the /command
                commands[commandName](tokens[0], args, (err, result) => {
                    if (err) {
                        return callback(err);
                    }

                    if (result) {
                        splitMarkdown[index] = splitMarkdown[index].replace(tokens[0], result);
                    }
