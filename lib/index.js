...
function _flipcoin(command, args, callback) {
	...
	result = Math.floor(responses.length * Math.random()); 
	answer = responses[result];

	if (args.toLowerCase() == 'heads' || args.toLowerCase() == 'tails') {
		if (answer.toString().toLowerCase().indexOf("heads") != -1 || answer.toString().toLowerCase().indexOf("tails") != -1){
			if (answer.toString().toLowerCase().indexOf(args.toLowerCase()) != -1){
				winmsg = 'YOU WON :thumbsup:';
			} else {
				winmsg = 'YOU LOST :cry:';
			}
		} else {
		...
		if (!args) {
			winmsg = "\n Next time, try calling it in the air";
		} else {
...
const commands = {
    ...
    'coinflip': function(args, callback) {
        _flipcoin('coinflip', args, callback);
    },
