...
const commands = {
    ...
    meme: function(args, callback) {
        ...
        if (argv.h || !argv.template) {
            return async.auto({
                fonts: function(callback) {
                    async.retry(function(callback) {
                        request.get('http://memegen.link/api/fonts', { json: true }, function(err, res, fonts) {
                            if (err) {
                                return callback(err);
                            }
                ...
                templates: function(callback) {
                    async.retry(function(callback) {
                        request.get('http://memegen.link/templates', { json: true }, function(err, res, templates) {
                            if (err) {
                                return callback(err);
                            }
                ...
                var help = 'Usage: /meme --template template [--top "top text"] [--bottom "bottom text"] [--font font]';
                help += '\n\nTemplates:\n';

                Object.keys(results.templates).map(name => {
                    return {
                        alias: results.templates[name].replace('http://memegen.link/templates/', ''),
                        name
                    };
                }).sort((a, b) => {
                    if (a.alias < b.alias) {
                    ...
                    help += `  ${template.alias}: ${template.name}\n`;
                });
                ...
                return callback(null, `/meme ${args}\n~~~\n${help}\n~~~`);
            });
        }
        ...
        var alt = `${argv.top || ''} ${argv.bottom || ''}`.trim();
        var url = `http://memegen.link/${argv.template}/${escape(argv.top)}/${escape(argv.bottom)}.jpg?font=${argv.font}`;
        callback(null, `![${alt}](${url})`);
    },
	...
	const dice = new roll();
	const orig = args;
	var show = orig.toString().endsWith('-show');
	args = args.replace('-show','').trim();
	...
	if (args === '--help' || args === '-h') {
		var help = '**Roll Command Usage Examples**\n';
		help += '  Use \`/roll\` to roll a value of 1-100\n';
		help += '  Use \`/roll d20\` to roll a single 20 sided die\n';
		help += '  Use \`/roll 2d20\` to roll two 20 sided dice\n';
		help += '  Use \`/roll 6d20b2\` to only count the best 2 values of six dice rolled\n';
		help += '  Use \`/roll 6d20w1\` to only count the worst value of six dice rolled\n';
		help += '  Use \`/roll d20+5\` to add 5 to the total of your roll\n';
		help += '  Use \`/roll 2d20+5d10\` to combine multiple sets of rolls\n';
		help += '  Use \`/roll 6d20 -show\` to show the results of the roll with the total\n';
		help += '  Use \`/roll -c\` to roll for a chance percentage (1-100)\n';
		help += '  Use \`/roll -bg\` to roll two 6 sided dice (boardgame style)\n';
		help += '  Use \`/roll -y\` to five two 6 sided dice (yahtzee style)\n';
        	return callback(null, `/roll ${orig}\n ${help}`);
		...
		const valid = dice.validate(args);
		if (!valid) {
        	        return callback(null, `/roll ${orig}\n The options \`${orig}\` are invalid. Use \`/roll --help\` for usage examples.`);
		} else {
			const value  = dice.roll(args);
			const rolled  = value.rolled;
			const numdice = rolled.toString().split(',');
			if (show && numdice.length > 1) {
        		        return callback(null, `/roll ${orig}\n:game_die: You rolled a total of ${value.result} using the following ${numdice.length} dice: [${rolled}]`);
			} else {
    ...
    woot: function(args, callback) {
        ...
        request.get(`http://api.woot.com/2/events.json?key=${process.env.WOOT_API_KEY}&select=Offers.Items.SalePrice,Offers.Title,Offers.Url`, { json: true }, function(err, res, events) {
            ...
            var _markdown = `/woot ${args}\n`;
            var searchResults = idx.search(args);

            searchResults.forEach(sr => {
                var offer = offers.find(o => o.Id === sr.ref);
                var maxPrice = offer.Items.sort((a, b) => b.SalePrice - a.SalePrice)[0].SalePrice;
                var minPrice = offer.Items.sort((a, b) => a.SalePrice - b.SalePrice)[0].SalePrice;
                var price = maxPrice === minPrice ? `$${maxPrice}` : `$${minPrice} - $${maxPrice}`;

                _markdown += `\n\n${offer.Title} for ${price}\n${offer.Url.split('?')[0]}`;
            });
