...
const commands = {
    ...
    define: function(args, callback) {
        if (!args || args === '--help' || args === '-h') {
            var help = 'Usage: /define search\n\n';
            help += 'Look up the definition of a word or phrase\n\n';
            help += 'Examples:\n';
            help += '  `/define meh` shows the definition of the word meh.\n';

            return callback(null, `/define ${args || ''}\n${help}`);
        } else {

            var _markdown = `/define ${args}\n`;
            // Get details from API
            async.retry(function(callback) {

            var headers = {
                'Accept': 'application/json',
                'app_id': process.env.OXFORD_APIKEY,
                'app_key': process.env.OXFORD_APPKEY
            };

            var options = {
                url: 'https://od-api.oxforddictionaries.com:443/api/v1/entries/en/' + encodeURIComponent(args),
                headers: headers,
                json: true
            };

            request.get(options, function(err, res, events) {
                if (err) {
                    return callback(err);
                }
                ...
                if (err == 'Error: 404') {
                    _markdown += '\nI was unable to find a definition for the specified search';
                    return callback(null, _markdown);
                } else if (err == 'Error: 403') {
                    _markdown += '\nError: Unable to complete lookup due to authorization failure';
                    return callback(null, _markdown);
                } else if (err) {
                    return callback(null, `${_markdown}\nError: Unable to contact the dictionary`);
                }

                events.results.forEach(e => {
                    e.lexicalEntries.forEach(le => {
                        _markdown += `\n**${args}** [*${le.lexicalCategory}*]\n`;
                        var entry = le.entries;
                        entry.forEach(ent => {
                            for (let count = 0; count < ent.senses.length; count++) {
                                var definition = ent.senses[count].definitions[0];
                                _markdown += ' ' + (count+1) + '. ' + definition + ' \n';

                                try {
                                    var example = ent.senses[count].examples[0].text;
                                    _markdown += '"' + example + '" \n\n';
                                }catch(e) {
                                    _markdown += 'No use case available\n\n';
                                }
                            }
                        });
                    });
