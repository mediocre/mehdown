let regExp = /(?:https?:\/\/(?:www\.)?)?((?:casemates|mediocre|mediocritee|meh|morningsave|pastadrop|sidedeal|stellacarmina)\.com)?\/(deals|polls)\/([-\w]+)/;
if (process.env.NODE_ENV !== 'production' && process.env.NODE_ENV !== 'staging') {
    // Make iframes for any localhost deal/poll links on debug
    regExp = /(?:https?:\/\/)?((?:127\.0\.0\.1|localhost):\d{4})?\/(deals|polls)\/([-\w]+)/;
}
